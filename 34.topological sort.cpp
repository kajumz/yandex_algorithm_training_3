...
void dfs(const int& v)//Поиск в глубину
{
    ...
    for (int i = 0; i < a[v].size(); i++)
        if (colors[a[v][i]] == 0) dfs(a[v][i]); else //Если не были в вершине то заходим нее
...
int main()
{
    //Ввод
    cin >> n >> m;
    for (int i = 0; i < n; i++) a.push_back(vector<int>());
    for (int i = 0; i < m; i++)
    {
        int x, y;
        cin >> x >> y;
    ...
    for (int i = 0; i < n; i++) colors[i] = 0;//Сначала все вершины покрасим в белый цвет

    for (int i = 0; i < n; i++)
        if (!colors[i]) dfs(i);

    //Вывод
    if (error) cout << -1; else
        while (!st.empty())
        {
            cout << st.top() + 1 << " ";
            st.pop();
